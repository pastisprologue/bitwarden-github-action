...
/**
 * Resolve the provided semver to exact version of `bw`.
 * This uses the npm registry and accepts latest, dist-tags or version ranges.
 * It's used to determine the cached version of `bw`.
 */
export async function resolve(version: string) {
  return (await registry.manifest(`@bitwarden/cli@${version}`)).version;
}

/**
 * Install `bw`, by version, using the packager.
 * Here you can provide any semver range or dist tag used in the registry.
 * It returns the path where Bitwarden is installed.
 */
...
/**
 * Install `bw`, by version, using npm or yarn.
...
export async function fromPackager(version: string, packager: string) {
  ...
  await cli.exec(tool, ["add", `@bitwarden/cli@${version}`], { cwd: root });
